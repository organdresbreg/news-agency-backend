...
class LLMRegistry:
    ...
    @classmethod
    def initialize(cls):
        """Initialize the LLM registry based on configured provider."""
        if cls.LLMS:  # Already initialized
            return

        provider = settings.LLM_PROVIDER
        logger.info("initializing_llm_registry", provider=provider)

        if provider == "groq":
            cls._initialize_groq()
        elif provider == "google":
            cls._initialize_google()
        else:  # openai (default)
            cls._initialize_openai()

    @classmethod
    def _initialize_openai(cls):
    ...
    @classmethod
    def _initialize_groq(cls):
        """Initialize Groq models (free tier)."""
        cls.LLMS = [
            {
